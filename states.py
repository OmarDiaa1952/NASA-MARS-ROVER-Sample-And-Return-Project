"""
Here we Apply the Concept of Finite State Machine Concept in the 3 files
    ...
    in the states.py file we have 13 state we will repesent each one in class
    1=> FindWall
    2=>FollowWall
    3=>TurnToWall():
    4=>AvoidWall
    ...
    6=>GoToSample
    7=>InitiatePickup
    8=>WaitForPickupInitiate
    9=>WaitForPickupFinish
    10=>GetUnstuck
    11=>ReturnHome
    12=>Stop
    13=>Park
    ...
    def execute(self, Rover):
        ...
        # Drive to sample
            ...
            if rock_pixs >= 1:
                ...
                # Yaw left if rock sample to left more than 20 deg
                if rock_heading >= 20:
                    Rover.throttle = 0
                    Rover.brake = 0
                    Rover.steer = self.YAW_LEFT_SET
                # Yaw right if rock sample to right more than -20 deg
                elif rock_heading <= -20:
                    Rover.throttle = 0
                    Rover.brake = 0
                    Rover.steer = self.YAW_RIGHT_SET
                # Otherwise drive at average rock sample heading
                elif (-20 < rock_heading < 20) or math.isnan(rock_heading):
                    Rover.brake = 0
                    Rover.throttle = self.THROTTLE_SET
